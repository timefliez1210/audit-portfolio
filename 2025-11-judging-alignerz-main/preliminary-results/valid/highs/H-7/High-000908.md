# [000908] Infinite Loop in calculateFeeAndNewAmountForOneTVS Causes Permanent DoS in mergeTVS and splitTVS
  
  ### Summary

`calculateFeeAndNewAmountForOneTVS()` contains a `for` loop that never increments the loop index `i`. As a result, `i` remains `0` forever and the loop **never terminates**, causing the function to **run out of gas and revert 100% of the time**. Because `mergeTVS` and `splitTVS` both rely on this function, these operations become **completely unusable**.
...
- The loop condition `i < length` is always true
- `i` is **never incremented**, so the loop body executes endlessly, resulting in a OOG error

### Internal Pre-conditions

- `mergeTVS` or `splitTVS` is called
- A different bug of `newAmounts` not being initialised needs to be fixed first
...
- `mergeTVS` and `splitTVS` become permanently unusable.
...
First, a different bug needs to be fixed, so in `FeesManager::calculateFeeAndNewAmountForOneTVS` add the following code to the top of the function:
...
Now when running the test, you will see that `mergeTVS` will revert with `EvmError: OutOfGas`:
...
Fix the loop by adding an increment:
