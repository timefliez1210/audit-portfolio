# [000600] Users Get No Dividend Through Invalid Equality Check
  
  ### Summary

In [`A26ZDividendDistributor.sol` lines 140–161](https://github.com/dualguard/2025-11-alignerz/blob/f7eeed88d91356484c02af6f38b71f27b790828c/protocol/src/contracts/A26ZDividendDistributor/A26ZDividendDistributor.sol#L140-L161), there is a check to verify whether the token associated with the allocation for a given `nftId` (which represents a TVS token) is the same as the `token` variable in the dividend distributor contract. If they match, the function will return 0 and set the unclaimable amount for that `nftId` to zero.

Based on the code in [`A26ZDividendDistributor.sol` lines 89–94](https://github.com/dualguard/2025-11-alignerz/blob/f7eeed88d91356484c02af6f38b71f27b790828c/protocol/src/contracts/A26ZDividendDistributor/A26ZDividendDistributor.sol#L89-L94), the `token` variable must be the TVS token, so it will always match the token associated with that `nftId` in the vesting contract. This logic leads to the user never receiving rewards because it always returns 0 and sets the unclaimable amount to zero.
...
The `token` variable in the dividend distributor contract is forced to match the TVS token, and the code checks if the allocation’s token for the `nftId` is the same. If they match, the function returns 0 and `unclaimableAmount` for that nftId always zero. Because the allocation’s token is always the TVS token, the condition always triggers, causing rewards to be permanently blocked for that `nftId`.

**Affected Code References:**

* Allocation token check: [`A26ZDividendDistributor.sol` lines 140–161](https://github.com/dualguard/2025-11-alignerz/blob/f7eeed88d91356484c02af6f38b71f27b790828c/protocol/src/contracts/A26ZDividendDistributor/A26ZDividendDistributor.sol#L140-L161)
...
1. The user attempts to claim their dividend by calling `getUnclaimedAmounts` on the dividend distributor contract after holding the TVS token for a certain period.
2. The unclaimable amount is always set to 0 due to the invalid logic in the contract.


### Impact

Invalid Equality Check Logic Prevents Users from Receiving Dividends for Holding TVS Token

### PoC

add test_InvalidEqualityLogicCheck to the test file
and run `forge test --mt test_InvalidEqualityLogicCheck -vvv`

disclaimer, i fix bug retrieving dynamic data on getUnclaimedAmount function to reveal the InvalidEqualityLogicCheck
...
contract AlignerzVestingProtocolTest is Test {
    ...
    function test_InvalidEqualityLogicCheck() public {
        ...
        // user get no dividend after waiting 365 days
        vm.prank(bidder);
        console.log("user dividend after waiting 365 days is %d", DividendDistributor.getUnclaimedAmounts(nftId));
...
the result show that user gets no dividend after waitng for vesting period on dividend contract.

```bash
Ran 1 test for test/AlignerzVestingProtocolTest.t.sol:AlignerzVestingProtocolTest
[PASS] test_InvalidEqualityLogicCheck() (gas: 23615196)
Logs:
  user dividend after waiting 365 days is 0

Suite result: ok. 1 passed; 0 failed; 0 skipped; finished in 4.68s (43.78ms CPU time)

Ran 1 test suite in 4.69s (4.68s CPU time): 1 tests passed, 0 failed, 0 skipped (1 total tests)
```
