# [000617] Lack of access control in distributeRewardTVS renders claimDeadline useless
  
  ### Summary

In `AlignerzVesting` contract, a `claimDeadline` is enforced. KOLs should only be able to call `claimRewardTVS` before the deadline, and the owner should be able to call `distributeRewardTVS` after the deadline. But, since there’s no access control on `distributeRewardTVS`, `claimDeadline` becomes completely useless, as a KOL can claim their rewards at any time, before or after the claim deadline.

Note: A similar issue exists in stablecoin distribution. Since they are both different functions for different functionalities, I submit them as 2 separate issues and they should not be confused.
...
See that `claimRewardTVS` function can be called by KOLs before the claimDeadline
...
But there is another function, `distributeRewardTVS` , that should be called by the owner to distribute the TVS that have not been yet claimed to the KOLs
...
Which also means KOLs can claim their rewards

- before the claimDeadline, by calling claimRewardTVS
- and also after the claimDeadline, by calling distributeRewardTVS
...
Add access control on `distributeRewardTVS`
